--殉道者之金钱攻略
function c77240177.initial_effect(c)
    --Activate
    local e1=Effect.CreateEffect(c)
    e1:SetType(EFFECT_TYPE_ACTIVATE)
    e1:SetCode(EVENT_SUMMON_SUCCESS)
    e1:SetTarget(c77240177.target)
    e1:SetOperation(c77240177.activate)
    c:RegisterEffect(e1)
...
function c77240177.target(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
    if not eg then return false end
    local tc=eg:GetFirst()
    if chkc then return chkc==tc end
    if chk==0 then return ep~=tp and  tc:IsOnField() end
end
function c77240177.activate(e,tp,eg,ep,ev,re,r,rp)
    local tc=eg:GetFirst()
    ...
    e1:SetCountLimit(1)
    e1:SetLabel(lv)
    e1:SetOperation(c77240177.desop)
    e1:SetReset(RESET_EVENT+RESETS_STANDARD+RESET_PHASE+PHASE_END,0)
    Duel.RegisterEffect(e1,tp)
end
function c77240177.desop(e,tp,eg,ep,ev,re,r,rp)
    local ct=e:GetLabel()
