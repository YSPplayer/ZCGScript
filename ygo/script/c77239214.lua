...
function c77239214.initial_effect(c)
	...
	e1:SetDescription(aux.Stringid(77239214,0))
	e1:SetCategory(CATEGORY_SPECIAL_SUMMON)
	e1:SetType(EFFECT_TYPE_SINGLE+EFFECT_TYPE_TRIGGER_O)
	e1:SetProperty(EFFECT_FLAG_DAMAGE_STEP+EFFECT_FLAG_DELAY)
	e1:SetCode(EVENT_DESTROYED)
	e1:SetCondition(c77239214.condition)
	e1:SetTarget(c77239214.target)
	e1:SetOperation(c77239214.operation)
	c:RegisterEffect(e1)
end
function c77239214.condition(e,tp,eg,ep,ev,re,r,rp)
...
function c77239214.target(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.GetLocationCount(tp,LOCATION_MZONE)>0
		and Duel.IsExistingMatchingCard(c77239214.filter,tp,LOCATION_DECK,0,1,nil,e,tp) end
...
function c77239214.operation(e,tp,eg,ep,ev,re,r,rp)
	...
	local g=Duel.SelectMatchingCard(tp,c77239214.filter,tp,LOCATION_DECK,0,1,ft,nil,e,tp)
