--植物的愤怒 青岩石龙
function c77239607.initial_effect(c)
	--synchro summon
	...
	e1:SetDescription(aux.Stringid(77239607,1)) 
	e1:SetCategory(CATEGORY_HANDES+CATEGORY_ATKCHANGE)
	e1:SetType(EFFECT_TYPE_IGNITION)
	e1:SetRange(LOCATION_MZONE)
	e1:SetTarget(c77239607.target)
	e1:SetOperation(c77239607.activate)
	c:RegisterEffect(e1)	
end
-----------------------------------------------------------------
function c77239607.target(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.GetFieldGroupCount(tp,LOCATION_HAND,0)>0
	and Duel.GetFieldGroupCount(tp,0,LOCATION_HAND)>0 end
	Duel.SetOperationInfo(0,CATEGORY_HANDES,nil,0,PLAYER_ALL,1)
end
function c77239607.activate(e,tp,eg,ep,ev,re,r,rp)
